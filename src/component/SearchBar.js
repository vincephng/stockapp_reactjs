...
class StockInfo extends Component{
 ...
 handleOnClick = (e) => {
    ...
    if(isValid == true){
      if(array.length == 0){
        this.setState({warningInput: 'Company is not found. Please Try Again.'})
 ...
 checkValid = () => {
     const input = this.state.ticker
        ...
        // if(!/^[a-zA-Z]+$/i.test(input)){
         this.setState({warningInput: "Please input valid symbol", suggestion:[]})
        }else if(!input){ 
...
render(){
    ...
    return( <>     
            <div className='stock-search'>
              <h1 className='stock-text' ><a href="" onClick={this.handleOnClick}>
                  Search</a></h1>
